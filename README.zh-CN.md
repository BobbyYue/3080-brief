...
0.10.0 要求用具体文字说明表达、内容取舍、来源忠实度和实际阅读顺序是否合格。结论、图和问答表应分别回答不同的读者问题。完整审阅、局部更新和明确指定的 Fast 模式都不能缺少有效记录；Fast 仍需披露为自检。原有单元、覆盖要求和审阅次数上限不变。升级后重新生成受影响的审阅记录，汇总时传入准备好的 `--reader-value-inputs` 文件。
