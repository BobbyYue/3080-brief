...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="3080-brief-eval-") as tmp:
        ...
        if not rich_text.startswith("<!-- Generated by 3080 Brief HTML Composer -->\n"):
            raise SystemExit("HTML renderer omitted the canonical composer signature")
        if not Path(str(rich_html_output) + ".contract.json").is_file() or not Path(str(rich_html_output) + ".build-receipt.json").is_file():
            raise SystemExit("HTML renderer omitted its contract or build receipt")
        for expected in (
        ...
        unsigned_html = tmp_path / "unsigned-rich.html"
        unsigned_html.write_text(rich_text.split("\n", 1)[1], encoding="utf-8")
        unsigned_failure = run(
            sys.executable,
            str(SCRIPTS / "validate_html_output.py"),
            str(unsigned_html),
            "--visual-spec", str(FIXTURES / "html-visual-spec.json"),
        ...
        if "missing the 3080 composer signature" not in unsigned_failure.stdout:
            raise SystemExit("HTML validator accepted an unsigned artifact")

        external_css_html = tmp_path / "external-css-rich.html"
        external_css_html.write_text(rich_text.replace("</style>", '@import url("https://example.invalid/theme.css");</style>', 1), encoding="utf-8")
        external_css_failure = run(
            sys.executable,
            str(SCRIPTS / "validate_html_output.py"),
            str(external_css_html),
            "--visual-spec", str(FIXTURES / "html-visual-spec.json"),
            "--design-plan", str(html_design),
            expect=1,
        )
        if "external CSS import or URL" not in external_css_failure.stdout:
            raise SystemExit("HTML validator accepted an external CSS dependency")
        ...
        stage_story_design = json.loads(html_design.read_text(encoding="utf-8"))
        stage_story_design["one_picture"]["support_position"] = "below"
        stage_story_design_path = tmp_path / "stage-story-html-design.json"
        stage_story_design_path.write_text(json.dumps(stage_story_design), encoding="utf-8")
        run(
            sys.executable,
            str(SCRIPTS / "build_html_brief.py"),
            str(FIXTURES / "html-brief.json"),
            ...
            "--design-plan", str(stage_story_design_path),
        )
        run(
            sys.executable,
            str(SCRIPTS / "validate_html_output.py"),
        ...
        for theme in themes:
            ...
            run(sys.executable, str(SCRIPTS / "build_html_brief.py"), str(FIXTURES / "html-brief.json"), str(themed_spec_path), str(themed_output), "--design-plan", str(html_design))
        ...
        run(sys.executable, str(SCRIPTS / "build_html_brief.py"), str(FIXTURES / "html-brief.json"), str(reading_room_path), str(reading_room_html), "--design-plan", str(html_design))
        ...
        validation_notes.write_text("Deterministic HTML and visual checks passed.\n", encoding="utf-8")
        contract_match = re.search(r'data-contract-id="([a-f0-9]{64})"', rich_text)
        if not contract_match:
            raise SystemExit("HTML runtime contract id was not embedded")
        full_page_preview = tmp_path / "full-page-preview.png"
        full_page_preview.write_bytes(b"synthetic full-page desktop preview")
        geometry_report = tmp_path / "geometry-report.json"
        geometry_report.write_text(json.dumps({
            "schema_version": 1,
        ...
        full_page_replay.write_text(json.dumps({
            "schema_version": 1,
            "reviewer_role": "full_page_visual",
            "contract_id": contract_match.group(1),
            "preview_sha256": hashlib.sha256(full_page_preview.read_bytes()).hexdigest(),
            "verdict": "PASS",
            "heading_path": ["Synthetic Evaluation Brief", "TLDR", "Why the result matters"],
            "first_screen_judgment": "The opening exposes the main decision before supporting detail.",
            "rhythm_assessment": "The report alternates prose and evidence without a component wall.",
            "dense_regions": [],
            "gates": {
        ...
        run(
            ...
            "--full-page-preview", str(full_page_preview),
            "--geometry-report", str(geometry_report),
            "--full-page-replay", str(full_page_replay),
    ...
    full_page_replay = config.get("full_page_visual_replay", {})
    if full_page_replay.get("position") != "after_visual_replay_before_blind_reader" or full_page_replay.get("input") != "full_page_desktop_screenshot_only":
        raise SystemExit("HTML full-page replay is not connected between visual and Blind Reader replay")
    expected_full_page_gates = {
    ...
    if set(full_page_replay.get("required_gates", [])) != expected_full_page_gates:
        raise SystemExit("HTML full-page replay gates are incomplete")
    html_render = config.get("html_render", {})
    if not all(html_render.get(key) is True for key in ("runtime_contract_required", "build_receipt_required", "full_page_preview_required", "geometry_report_required", "full_page_visual_replay_required")):
        raise SystemExit("HTML runtime and full-page evidence gates are incomplete")
