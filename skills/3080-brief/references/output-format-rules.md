...
| HTML file | New self-contained HTML file | bundled 3080 HTML renderer and validator |
...
Feishu and HTML render from the same reviewed `brief.json` and `visual_spec.json`. Their native renderers may differ, but they may not drift in claims, hierarchy, selected theme, semantic meaning, or body visual references.
...
- HTML / local file: generated `.html` file path.
