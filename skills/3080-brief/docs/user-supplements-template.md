# 3080-brief Skill 补充修改文档

用途：这份文档用于沉淀你后续对 `3080-brief` skill 的补充、限制、偏好和案例。每天北京时间 08:00-11:00 的定时检查会把下方同步区内容更新到运行规则文件，并同步到各 skill roots。

填写方式：

- 只修改 `SYNC_TO_SKILL_START` 和 `SYNC_TO_SKILL_END` 之间的内容。
- 保留章节标题，便于同步后的 skill 仍然结构清晰。
- 同步区是运行时常读规则，只放硬约束、偏好和索引；长方法论、案例和解释优先放到 `references/`。
- 不确定的规则先写在“待确认问题”，确认后再移动到对应章节。
- 涉及数据、案例、原文链接、白板链接时，请补充可追溯来源。
...
同步脚本只读取上方两个 HTML 标记之间的内容，更新唯一事实源中的运行规则文件，并在发布前执行本地 eval。`SKILL.md` 只保留补充规则索引，不再创建其他 skill root 副本。

可移植路径：

- Skill: `${CODEX_HOME:-$HOME/.codex}/skills/3080-brief/SKILL.md`
- 同步脚本: `scripts/sync_user_supplements.sh`
- 自动同步属于用户本机配置，不随公开 skill 分发。
